...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  let payload: { decision?: string; comment?: string; confirmed?: boolean } = {};
  ...
  if (payload.confirmed !== true) return NextResponse.json({ error: "Confirm Approve or Reject before recording this decision." }, { status: 400, headers: privateHeaders });
