...
export async function GET(request: Request, { params }: { params: Promise<{ token: string; attachmentId: string }> }) {
  ...
  try { const url = await getProductionApprovalFileUrl({ kind: "external", token }, attachmentId, "download"); return NextResponse.redirect(url, { status: 302, headers: { "Cache-Control": "private, no-store, max-age=0" } }); }
