...
function resolveTargetLanguage(payload: TranslateBatchPayload) {
...
export async function POST(request: Request) {
  ...
  let payload: TranslateBatchPayload = {};

  try {
    payload = (await request.json()) as TranslateBatchPayload;
  } catch {
  ...
  if (items.length > MAX_TRANSLATION_BATCH_ITEMS) {
    return NextResponse.json(
      {
        error: `Too many messages to translate at once. Keep it under ${MAX_TRANSLATION_BATCH_ITEMS} text blocks.`,
      },
      { status: 400 },
    );
  }
  ...
  if (totalCharacters > MAX_TRANSLATION_BATCH_CHARACTERS) {
    return NextResponse.json(
      {
        error: `Conversation text is too long to translate at once. Keep it under ${MAX_TRANSLATION_BATCH_CHARACTERS} characters.`,
      },
      { status: 400 },
    );
  }
  ...
  const rateLimit = checkAiRateLimit({
    key: `ai:translate-batch:${user.id}`,
    limit: 8,
  });
  ...
  try {
    const result = await translateTextsWithOpenAI({
      items,
