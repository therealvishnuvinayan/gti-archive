...
export function UpdateList({ title, items }: UpdateListProps) {
  return (
    <article className="flex min-h-[220px] min-w-0 flex-col rounded-[24px] bg-card p-5 shadow-[0_18px_45px_rgba(23,39,28,0.05)] sm:p-6">
      ...
      {items.length > 0 ? (
        <ul className="dashboard-scroll-thin -mr-2 max-h-[260px] space-y-2 overflow-y-auto pr-2">
          {items.map((item) => {
        ...
        <div className="grid min-h-[120px] place-items-center rounded-[18px] border border-dashed border-[#dce6dd] bg-[#fbfcfa] px-4 py-6 text-center">
