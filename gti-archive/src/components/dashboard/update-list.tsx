...
export function UpdateList({ title, items }: UpdateListProps) {
  return (
    <article className="rounded-[24px] bg-card p-5 shadow-[0_18px_45px_rgba(23,39,28,0.05)] sm:p-6">
      <h2 className="mb-5 text-[18px] font-extrabold text-[#111712]">{title}</h2>
        ...
        {items.map((item) => {
          ...
          return (
            <li key={`${item.title}-${item.project}`} className="flex items-start gap-3">
              ...
              <div>
                <p className={`text-lg font-bold leading-tight ${tone.title}`}>
                  {item.title}
                </p>
                <p className="text-sm text-[#464d47]">{item.project}</p>
              </div>
            </li>
          );
        })}
